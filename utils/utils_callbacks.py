...
class CallBackLoggingKD(object):
    ...
    def __call__(self, global_step, loss: AverageMeter, loss1: AverageMeter, loss2: AverageMeter, epoch: int):
        if self.rank == 0 and global_step > 0 and global_step % self.frequent == 0:
            if self.init:
                try:
                    speed: float = self.frequent * self.batch_size / (time.time() - self.tic)
                ...
                if self.writer is not None:
                    self.writer.add_scalar('time_for_end', time_for_end, global_step)
                    self.writer.add_scalar('loss', loss.avg, global_step)
                msg = "Speed %.2f samples/sec   Loss %.4f ArcLoss %.4f MSE %.4f  Epoch: %d   Global Step: %d   Required: %1.f hours" % (
                    speed_total, loss.avg, loss1.avg, loss2.avg, epoch, global_step, time_for_end
                )
                logging.info(msg)
                loss.reset()
                loss1.reset()
